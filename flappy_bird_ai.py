...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, position_x):
        ...
        self.speed = 5
        self.vertical_speed = random.choice([5, -5])
    ...
    def move(self):
        ...
        if self.height > SCREEN_HEIGHT - 360:
            self.vertical_speed = -5
        elif self.height < 100:
            self.vertical_speed = 5

        self.height += self.vertical_speed
        self.top_pipe_position = self.height - self.top_pipe.get_height()
        self.ground_pipe_position = self.height + self.pipes_distance
...
def draw_screen(screen, birds, pipes, ground, score):
    ...
    text = POINTS_FONT.render(f"Generation: {generation}", True, (0, 0, 0))
    screen.blit(text, (SCREEN_WIDTH - text.get_width(), text.get_height()))
    ...
    while True:
        ...
        for i, bird in enumerate(birds):
            ...
            inputs = (bird.x,
                      ...
                      abs(bird.y - pipes[pipe_index].ground_pipe_position),
                      )
            outputs = neural_networks[i].activate(inputs)
            if outputs[0] > 0.5:
                bird.jump()
...
if __name__ == "__main__":
    CONFIG_PATH = ROOT_FOLDER / "config.txt"
    run(CONFIG_PATH)
